...
public class Queue extends Command{
	...
	public void runCommand(Member author, TextChannel channel, Guild guild, String[] msgParams) {
        ...
        if(manager.player.getPlayingTrack() == null) {
            ...
            messages.append("**Musica Atual: **[" + manager.player.getPlayingTrack().getInfo().title+"]("+ manager.player.getPlayingTrack().getInfo().uri + ")\n\n");
        }
        ...
        if(songs.isEmpty()) {
			...
			if(hook() == null) 
				...
				messageId = hook().editOriginalEmbeds(pagedEmbed.getCurrentPage()).setActionRow(comps).complete().getId();
