...
public class Config extends Command {
	...
	public void runCommand(Member author, TextChannel channel, Guild guild, String[] msgParams) {
		...
		if(msgParams.length > 1){
					...
					switch(subsubcmd) {
						case "msg":
							if(msgParams.length > 3) {
								...
								eb.setDescription("É necessário que você escreva as mensagens de boas vindas no comando.");
							}
							break;
						case "channel":
							if(msgParams.length > 3){
								TextChannel welcomeChannel = guild.getTextChannelById(msgParams[3].replaceAll("[<#>]", ""));
								if(welcomeChannel != null) {
									cfg.setWelcomeChannel(welcomeChannel.getId());
									updated = true;
									eb.setDescription("O canal de boas vindas foi setado para " + welcomeChannel.getAsMention());
								}
								else {
				...
				case "alertroles":
					if(msgParams.length == 5){
						...
						if(role1 != null && role2 != null && role3 != null){
							...
							updated = true;
							eb.setDescription("Alert Roles: " + role1.getAsMention() +" "+ role2.getAsMention() +" "+ role3.getAsMention());
						}
						else {
							eb.setDescription("Um dos cargos específicados não existe ou não foi encontrado. É necessário haver 1 espaço entre cada cargo.");
						}
					}
					else {
						eb.setDescription("É necessário que você especifique todos os 3 cargos de alerta com 1 espaço entre cada.");
					}
					break;
				case "prefix":
					if(msgParams.length > 2){
			...
			String desc = builder.toString().replace("<@&null>", "Nenhum").replace("<#null>", "Nenhum").replace("null", "Nenhum");
